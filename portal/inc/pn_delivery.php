...
function sendUserPN($empf, $betreff, $index)
{
    ...
    if (!isset($empfaenger) || empty ($empfaenger)) {
        ...
        $submit = sendPN($_SESSION["user_id"], $empfaenger, $betreff, $index);
...
function sendPN($abs, $empf, $betreff, $index)
{
    ...
    $send = @mysql_query("INSERT INTO $skrupel_portal_messages (absender,empfaenger,title,text,time) VALUES ($abs,$empf,'$betreff','$index','" . time() . "')");
}
...
function sendAnfrage($from, $thema, $inhalt)
{
    ...
    $submit = sendPN($abs, 0, $betreff, $index);
}
